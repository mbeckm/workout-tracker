...
11. **Truncation is a peer row.** When a list is truncated, the overflow control is a **full list row** on the same lanes (44 still with `+`, `n more exercise(s)` 17, chevron down) — not a floating `+n more` caption. Tap expands **in place**; expanded state ends with Show less (chevron up). Do not open a sheet just to reveal the rest of the Home list.
...
| --- | --- | --- | --- |
| Home | Day name (under `Next Workout` 28) | Identity: day title + meta only (`n exercises · ~Xm`) — no focus/region subtitle. Exercise list as **one object** (still + name + prescription meta). Truncate after ~4 rows with a **full list row**: 44 still with `+`, `n more exercise(s)`, chevron down — same lanes as exercise rows. Tap expands in place (show all + Show less). Black Start sits with the list (tighter than the week). Week = **amount** (`n of m` + dots; **completed dots are green**), with clear air above the tab bar. Tap list/day → preview, not log. | Completed week dots + checks only. |
...
| Home preview | Sheet | Still + name + `n × reps`; Start black starts that day | None |
